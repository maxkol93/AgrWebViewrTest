...
// Функция для получения списка всех загруженных моделей с Cloudinary
async function fetchUserModelsFromCloudinary() {
    try {
        // Вместо Search API используем прямой URL к JSON-файлу
        const cloudName = 'dd3nhch0g';
        
        // Получаем URL к JSON-файлу из разных источников в порядке приоритета:
        // 1. HTML meta тег с id="models-json-url"
        // 2. localStorage (если был сохранен ранее)
        // 3. Значение по умолчанию
        let jsonUrl;
        
        // Проверяем наличие мета-тега с URL
        const metaElement = document.getElementById('models-json-url');
        if (metaElement && metaElement.getAttribute('content')) {
            jsonUrl = metaElement.getAttribute('content');
            // Сохраняем в localStorage для будущих загрузок
            localStorage.setItem('models_json_url', jsonUrl);
            console.log('Используем URL JSON из HTML:', jsonUrl);
        }
        // Если URL не найден в HTML, проверяем localStorage
        else if (localStorage.getItem('models_json_url')) {
            jsonUrl = localStorage.getItem('models_json_url');
            console.log('Используем URL JSON из localStorage:', jsonUrl);
        }
        // Если URL всё еще не найден, используем значение по умолчанию
        else {
            jsonUrl = `https://res.cloudinary.com/dd3nhch0g/raw/upload/v1746464199/3d_models_lo0sww.JSON`;
            console.log('Используем URL JSON по умолчанию:', jsonUrl);
        }
        
        // Отображаем текущий URL для администратора в консоли
        console.log('Текущий URL JSON:', jsonUrl);
        console.log('Для изменения URL обновите значение мета-тега "models-json-url" в HTML файле');
        console.log('Или используйте функцию в консоли: updateJsonUrl("https://new-url.json")');
        
        // Отправляем запрос
        const response = await fetch(jsonUrl);
        if (!response.ok) {
            console.log('Не удалось получить список моделей, загружаем из localStorage');
            loadModelsFromLocalStorage();
            return;
        }
        ...
        if (!data.resources || data.resources.length === 0) {
            console.log('Список моделей пуст');
            loadModelsFromLocalStorage();
            return;
        }
        
        console.log('Получены модели из Cloudinary:', data.resources.length);
        
        // Очищаем существующий список моделей
        clearModelSelector();
        
        // Преобразуем данные в формат userModels
        const models = data.resources.map(resource => {
            // Используем прямой URL из JSON вместо генерации
            return {
                url: resource.url,
                name: resource.name || resource.public_id.split('/').pop(),
                uploadedAt: new Date().toISOString(),
        ...
        console.error('Ошибка при получении моделей из Cloudinary:', error);
        loadModelsFromLocalStorage();
    }
}
...
async function init() {
    // Проверяем наличие мета-тега для URL JSON-файла
    const metaElement = document.getElementById('models-json-url');
    if (!metaElement) {
        console.warn('Мета-тег "models-json-url" не найден в HTML. Будет использован URL из localStorage или значение по умолчанию.');
    }
...
// Функция для загрузки скрипта Cloudinary Upload Widget
function loadCloudinaryScript() {
    return new Promise((resolve, reject) => {
        ...
        script.onerror = () => reject(new Error('Не удалось загрузить Cloudinary Upload Widget'));
        document.head.appendChild(script);
    });
}

// Функция для открытия виджета загрузки Cloudinary
async function openCloudinaryUploader() {
    try {
        // Удаляем проверку авторизации и подписки отсюда, так как она уже есть в обработчике кнопки
        // Эта проверка вызывала дублирование модального окна
        
        const cloudinary = await loadCloudinaryScript();
        
        // Создаем виджет загрузки
        const uploadWidget = cloudinary.createUploadWidget({
            cloudName: 'dd3nhch0g', // Ваш cloudName с Cloudinary
            uploadPreset: '3d_models', // Убедитесь, что этот пресет настроен для неподписанных загрузок
            folder: '3d_models', // Папка для сохранения моделей
            resourceType: 'raw', // Для загрузки файлов форматов GLB/GLTF
            sources: ['local', 'url', 'camera', 'google_drive', 'dropbox'],
            maxFileSize: 10000000, // 10МБ в байтах
            multiple: false,
            tags: ['3d_models'], // Добавляем тег для моделей - ВАЖНО для автоматического поиска
            clientAllowedFormats: ['glb', 'gltf'], // Удаляем FBX из разрешенных форматов
            ...
            text: {
                'ru': {
                    ...
                    'local': {
                        'browse': 'Выбрать файл',
                        'dd_title_single': 'Перетащите 3D модель сюда'
                    },
                    'menu': {
                        'files': 'Мои файлы'
                    }
            ...
            if (!error && result && result.event === 'success') {
                // Получаем имя файла из результата загрузки
                const filename = result.info.original_filename || '';
                // Извлекаем расширение файла, проверяем разными способами
                let format = '';
                
                // Способ 1: из оригинального имени файла
                if (filename && filename.includes('.')) {
                    format = filename.split('.').pop().toLowerCase();
                }
                
                // Способ 2: если в имени файла нет расширения, пытаемся получить из public_id
                if (!format && result.info.public_id) {
                    const parts = result.info.public_id.split('.');
                    if (parts.length > 1) {
                        format = parts.pop().toLowerCase();
                    }
                }
                
                // Способ 3: проверяем, есть ли формат в ответе Cloudinary
                if (!format && result.info.format) {
                    format = result.info.format.toLowerCase();
                }
                
                // Создаем информацию о модели
                const modelInfo = {
                    url: result.info.secure_url,
                    name: filename || result.info.public_id.split('/').pop(),
                    uploadedAt: new Date().toISOString(),
                    size: result.info.bytes || 0,
                    format: format
                };
                
                console.log('Модель успешно загружена:', modelInfo);
                
                // Проверяем формат
                if (format !== 'glb' && format !== 'gltf') {
                    console.error(`Ошибка: Недопустимый формат файла "${format}". Поддерживаются только GLB и GLTF.`);
                    document.querySelector('.loading').textContent = `Ошибка: Недопустимый формат файла "${format}". Поддерживаются только GLB и GLTF.`;
                    document.querySelector('.loading').style.display = 'block';
                    setTimeout(() => {
                        document.querySelector('.loading').style.display = 'none';
                    }, 5000);
                ...
                userModels.push(modelInfo);
                
                // Добавляем модель в выпадающий список
                addModelToSelector(modelInfo);
                
                // Загружаем новую модель
                currentModelPath = modelInfo.url;
                loadModel();
                
                // Сохраняем список моделей в localStorage
                localStorage.setItem('userModels', JSON.stringify(userModels));
                
                // Создаем JSON для добавления в файл на Cloudinary
                const jsonToAdd = {
                ...
                // Получаем текущий URL JSON-файла
                const metaElement = document.getElementById('models-json-url');
                let currentJsonUrl = '';
                
                if (metaElement && metaElement.getAttribute('content')) {
                    currentJsonUrl = metaElement.getAttribute('content');
                } else {
                    currentJsonUrl = localStorage.getItem('models_json_url') || 
                                    'https://res.cloudinary.com/dd3nhch0g/raw/upload/v1746464199/3d_models_lo0sww.JSON';
                }
                
                // Выводим информацию для обновления JSON только в консоль (для администратора)
                console.log('ВАЖНО: После загрузки новой модели обновите JSON-файл на Cloudinary. ' +
                            'Добавьте в массив resources следующий объект:', JSON.stringify(jsonToAdd, null, 2));
                console.log(`Текущий URL JSON: ${currentJsonUrl}`);
                console.log('После загрузки обновленного JSON, обновите мета-тег "models-json-url" в HTML файле');
                console.log('Или используйте функцию в консоли: updateJsonUrl("https://new-url.json")');
                
            } else if (error) {
                console.error('Ошибка загрузки модели:', error);
                document.querySelector('.loading').textContent = 'Ошибка загрузки модели!';
                document.querySelector('.loading').style.display = 'block';
                setTimeout(() => {
                    document.querySelector('.loading').style.display = 'none';
                }, 3000);
...
// Вспомогательная функция для администратора (не используется в пользовательском интерфейсе)
function createCloudinaryUpdateHelper(modelJson) {
    // Функция оставлена только для возможного использования в будущем
    console.log('Помощник обновления JSON не отображается пользователям');
}

// Функция для обновления URL JSON-файла в HTML и localStorage
function updateJsonUrl(newUrl) {
    ...
    try {
        ...
        const metaElement = document.getElementById('models-json-url');
        if (metaElement) {
            metaElement.setAttribute('content', newUrl);
            // Сохраняем в localStorage
            localStorage.setItem('models_json_url', newUrl);
            console.log('URL JSON-файла обновлен:', newUrl);
            return true;
        } else {
            console.error('Мета-тег "models-json-url" не найден в HTML');
            return false;
        }
    } catch (error) {
        console.error('Ошибка при обновлении URL JSON-файла:', error);
        return false;
    }
}

// Добавляем функцию в глобальную область для использования в консоли администратором
window.updateJsonUrl = updateJsonUrl;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (initSupabase()) {
        // Загружаем модели из Supabase
        fetchModelsFromSupabase();
    } else {
        // Если не удалось инициализировать Supabase, загружаем из Cloudinary или localStorage
        fetchUserModelsFromCloudinary();
    }
